...
This project is designed to automate the process of joining Discord servers and boosting them using multiple tokens. It utilizes the `discord.js-selfbot-v13` library for Discord interactions, `AzCaptcha API` for solving captchas, and proxies for managing multiple accounts.
...
- `azcaptcha_key`: Your API key from AzCaptcha, used for solving captchas automatically.
...
Before running the script, ensure you have Node.js installed on your system. You will also need an AzCaptcha API key and a list of Discord tokens.
...
- If captchas are encountered, it uses the AzCaptcha API service to solve them.
